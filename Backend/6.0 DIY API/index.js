...
app.get('/random', async (req, res) => {
  ...
  console.log(`Random joke number: ${jokeNumber}`);
  const randomJoke = jokes[jokeNumber];
...
app.get('/jokes/:id', async (req,res) =>{
  ...
  res.json(jokes.find(joke => joke.id === jokeId) || { error: "Joke not found" })
});
...
app.get('/filter', async (req, res) => {
  ...
  console.log(`Filtering jokes by type: ${jokeType}`);
...
app.post('/jokes', async (req, res) => {
  ...
  jokes.push(newJoke);
  res.json(newJoke);
  console.log(`New joke added: ${JSON.stringify(newJoke)}`);
});
...
app.put("/jokes/:id",async (req, res) => {
  ...
  const jokeText = req.body.text;
  const jokeType = req.body.type;
  jokes[jokeId-1].jokeText = jokeText;
  jokes[jokeId-1].jokeType = jokeType;
  res.json(jokes[jokeId-1]);
  console.log(`Joke with ID ${jokeId} updated: ${JSON.stringify(jokes[jokeId-1])}`);
...
app.patch('/jokes/:id', (req,res)=>{
  const jokeId = parseInt(req.params.id);
  console.log(`Partially updating joke with ID: ${jokeId}`);
  if(req.body.text){
  ...
  res.json(jokes[jokeId-1]);
  console.log(`Joke with ID ${jokeId} partially updated: ${JSON.stringify(jokes[jokeId-1])}`);
...
app.delete("/all",(req,res)=>{
  ...
  return res
    .status(403)
    .json({ error: "Unauthorized access. Invalid master key." });
})
